...
async function startServer() {
  ...
  const processSignalForAllUsers = async (text: string) => {
    ...
    if (type) {
      ...
      // Improved pair detection: look for 3-10 chars that aren't common command words
      const words = msg.split(/\s+/);
      let pair = 'EURUSD';
      const commonWords = ['buy', 'sell', 'compra', 'venta', 'sl', 'tp', 'at', '@', 'en', 'in'];
      
      for (const word of words) {
        const cleanWord = word.replace(/[^a-z0-9]/g, '');
        if (cleanWord.length >= 3 && cleanWord.length <= 10 && !commonWords.includes(cleanWord)) {
          pair = cleanWord.toUpperCase();
          break;
        }
      }
  ...
  app.post('/api/signals/clear', (req, res) => {
    ...
    db.prepare('DELETE FROM signals WHERE user_id = ?').run(user_id);
    res.json({ success: true });
  });
