...
export default function App() {
  ...
  const [bridgeType, setBridgeType] = useState<'python' | 'nodejs'>('python');
  ...
  const saveSettings = async () => {
    ...
    try {
      ...
      if (res.ok) {
        // Update local user object to keep settings in sync
        const updatedUser = { ...user, ...settings };
        setUser(updatedUser);
        localStorage.setItem('adberry_user', JSON.stringify(updatedUser));
        alert('✅ Ajustes guardados permanentemente en la base de datos');
                  ...
                  <div className="bg-[#0F172A]/40 backdrop-blur-md border border-white/5 p-6 rounded-3xl shadow-2xl flex flex-col">
                    <div className="flex items-center justify-between mb-6">
                      ...
                      <div className="flex items-center gap-3">
                        <button 
                          onClick={async () => {
                            if (window.confirm('¿Limpiar todas las señales del terminal?')) {
                          ...
                          className="px-2 py-1 bg-slate-900 border border-white/5 rounded text-[8px] font-black text-slate-500 hover:text-rose-500 transition-all uppercase tracking-widest"
                        >
                          Limpiar
                        </button>
              ...
              <div className="p-8 space-y-6 overflow-y-auto max-h-[75vh] custom-scrollbar">
                <div className="space-y-6">
                  <div className="bg-rose-500/10 border border-rose-500/20 p-4 rounded-2xl">
                    <p className="text-[10px] text-rose-400 font-bold uppercase tracking-wider mb-2">Selecciona tu método de conexión</p>
                    <div className="flex gap-2">
                      <button 
                        onClick={() => setBridgeType('python')}
                        className={`flex-1 py-2 rounded-xl text-[10px] font-black transition-all ${bridgeType === 'python' ? 'bg-rose-500 text-white' : 'bg-slate-800 text-slate-400'}`}
                      >
                        PYTHON (RECOMENDADO)
                      </button>
                      <button 
                        onClick={() => setBridgeType('nodejs')}
                        className={`flex-1 py-2 rounded-xl text-[10px] font-black transition-all ${bridgeType === 'nodejs' ? 'bg-rose-500 text-white' : 'bg-slate-800 text-slate-400'}`}
                      >
                        NODE.JS (NUEVO)
                      </button>
                    </div>
                    ...
                    <>
                      ...
                      <div className="space-y-2">
                        <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest flex items-center gap-2">
                          2. Script de Conexión (adberry_bridge.py)
                          <span className="text-[8px] text-amber-500 normal-case font-medium">⚠️ Desactiva el traductor de tu navegador antes de copiar</span>
                        </label>
                        <div className="bg-slate-900 p-4 rounded-xl border border-white/5 relative group">
                          <pre translate="no" className="text-[9px] text-emerald-400 font-mono leading-relaxed overflow-x-auto">
...
def execute_trade(signal):
    ...
    request = {
        ...
        "symbol": symbol,
        "volume": float(${settings.lot_size || 0.01}),
        "type": order_type,
        "price": price,
        "sl": float(signal['sl']) if signal['sl'] else 0.0,
        "tp": float(signal['tp']) if signal['tp'] else 0.0,
        "magic": 123456,
        "comment": "Adberry Signal",
        "type_time": mt5.ORDER_TIME_GTC,
        "type_filling": mt5.ORDER_FILLING_IOC,
    }
    result = mt5.order_send(request)
    
    # Si falla por filling (común en IC Markets), probar con FOK
...
print(f"🚀 Puente Python Activo - Cuenta: {ACCOUNT_ID}")
...
while True:
    try:
        res = requests.get(f"{API_URL}/api/mt5/pending/{ACCOUNT_ID}")
        if res.status_code == 200:
            for s in res.json().get('signals', []):
                print(f"📝 Ejecutando {s['type']} {s['pair']}...")
                r = execute_trade(s)
                if r and r.retcode == mt5.TRADE_RETCODE_DONE:
                    print(f"✅ Éxito: Operación abierta")
                    requests.post(f"{API_URL}/api/mt5/confirm", json={"signal_id": s['id']})
                else:
                    err = r.comment if r else "Error de conexión/símbolo"
                    code = r.retcode if r else "N/A"
                    print(f"❌ Error en MT5: {err} (Código: {code})")
                    # Si el símbolo no existe, confirmamos para que no se repita infinitamente
                    if code == 10017: # TRADE_RETCODE_INVALID_SYMBOL
                        print(f"⚠️ Símbolo {s['pair']} no válido. Saltando...")
                        requests.post(f"{API_URL}/api/mt5/confirm", json={"signal_id": s['id']})
    ...
    time.sleep(5)`}
                    ...
                    <>
                      <div className="space-y-2">
                        <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest">1. Instalar Dependencias</label>
                        <div className="bg-slate-900 p-3 rounded-xl border border-white/5">
                          <code className="text-xs text-rose-400 font-mono">npm install socket.io-client axios</code>
                        </div>
                      </div>

                      <div className="space-y-2">
                        <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest flex items-center gap-2">
                          2. Script de Conexión (adberry_bridge.js)
                          <span className="text-[8px] text-amber-500 normal-case font-medium">🚀 Real-time con Socket.io</span>
                        </label>
                        <div className="bg-slate-900 p-4 rounded-xl border border-white/5 relative group">
                          <pre translate="no" className="text-[9px] text-blue-400 font-mono leading-relaxed overflow-x-auto">
{`const { io } = require("socket.io-client");
const axios = require("axios");
const { exec } = require("child_process");

const API_URL = "${window.location.origin}";
const ACCOUNT_ID = "${settings.mt5_account}";
const LOT_SIZE = ${settings.lot_size};

console.log("🚀 Iniciando Puente Node.js...");
const socket = io(API_URL);

socket.on("connect", () => {
  console.log("✅ Conectado al servidor Adberry");
});

socket.on("new_signal", async (signal) => {
  console.log("🔔 Nueva señal recibida:", signal.type, signal.pair);
...
if res and res.retcode == 10030:
    request["type_filling"] = mt5.ORDER_FILLING_FOK
    res = mt5.order_send(request)
print(res.retcode if res else "Error")
mt5.shutdown()
\`;

  exec(\`python -c "\${pythonCmd.replace(/"/g, '\\\\"')}"\`, async (error, stdout) => {
    const code = stdout.trim();
    if (code === "10009") {
      console.log("✅ Operación exitosa en MT5");
      await axios.post(\`\${API_URL}/api/mt5/confirm\`, { signal_id: signal.id });
    } else {
      console.log("❌ Error en MT5. Código:", code);
      if (code === "10017") {
         console.log("⚠️ Símbolo no válido. Saltando...");
         await axios.post(\`\${API_URL}/api/mt5/confirm\`, { signal_id: signal.id });
      }
    }
  });
