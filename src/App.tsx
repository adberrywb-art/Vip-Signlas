...
def execute_trade(signal):
    ...
    symbol = signal['pair']
    # Asegurar que el símbolo sea válido en MT5 (ej: EURUSD)
    mt5.symbol_select(symbol, True)
    
    order_type = mt5.ORDER_TYPE_BUY if signal['type'] == 'BUY' else mt5.ORDER_TYPE_SELL
    tick = mt5.symbol_info_tick(symbol)
    if tick is None:
        print(f"Error: No se pudo obtener el precio para {symbol}")
        return None
...
print(f"🚀 Iniciando Adberry Bridge para cuenta: {ACCOUNT_ID}")
print(f"🔗 Conectando a: {API_URL}")

if not mt5.initialize():
    print("❌ Error al inicializar MT5. Asegúrate de que esté abierto.")
    quit()
...
while True:
    try:
        ...
        if response.status_code != 200:
            print(f"⚠️ Error del servidor ({response.status_code}): {response.text[:100]}")
            time.sleep(10)
            continue
        ...
        if signals:
            print(f"🔔 Se encontraron {len(signals)} señales nuevas.")
            
        for signal in signals:
            print(f"📝 Procesando {signal['type']} en {signal['pair']}...")
            res = execute_trade(signal)
            if res and res.retcode == mt5.TRADE_RETCODE_DONE:
                print(f"✅ Operación ejecutada con éxito en MT5.")
                requests.post(f"{API_URL}/api/mt5/confirm", json={"signal_id": signal['id']})
            else:
                error_msg = res.comment if res else "Error desconocido"
                print(f"❌ Error al ejecutar en MT5: {error_msg}")
                
    except json.decoder.JSONDecodeError:
        print("❌ Error: El servidor no devolvió JSON. Verifica la URL en la configuración.")
